    ...
    stages{
        stage("A"){
            steps{
                echo "========executing A========"
                sh '''
                 "aws ecr get-login-password --region us-east-1 | docker login -u AWS -p $(aws ecr get-login-password) https://$(aws sts get-caller-identity --query 'Account' --output text).dkr.ecr.${AWS_DEFAULT_REGION}.amazonaws.com"
