    ...
    stages {
        ...
        stage('Push') {
            steps {
                ...
                sh 'docker push 250408828727.dkr.ecr.eu-central-1.amazonaws.com/flaskapp:$GIT_COMMIT'
            }
           }
         stage('Deploy') {
            steps {
                sh 'helm upgrade flaskapp helm/flaskapp --install --wait --set deployment.tag=$GIT_COMMIT'
            }
        }
     }
  }
